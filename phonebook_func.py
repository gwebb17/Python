...
def center_window(self, w, h): #pass in tkinter frame(master) reference and w/h
    ...
    screen_width = self.master.winfo_screenwidth() #self.master is targeting main window, winfo... is built in method
...
#catch if the user clicks on the windows upperight X to ensure they want to close
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Okay to exit application?"):
    #this closes app
        self.master.destroy()
        os._exit(0) #fully deletes all widgets from users memory, so it is freed up when they close program

#=====================================================================
def create_db(self):
    conn = sqlite3.connect('db_phonebook.db')
    with conn:
        cur = conn.cursor() #required to make sql queries/statements
        ...
        #commit to save changes/close db connection
        conn.commit()
    conn.close()
    first_run(self)
...
def count_records(cur):
    count = ""
    cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""") #count up all rows and return it
    count = cur.fetchone()[0] #call first index 0 from the tuple in above function first_run. 
    return cur,count
...
def onSelect(self,event):
    ...
    with conn:
        ...
        for data in varBody:
            self.txt_fname.delete(0,END) #0 - END means from beginning to the end of the tuple this line deletes that
            self.txt_fname.insert(0,data[0]) #this inserts the data user types into the row we just deleted from
...
def addToList(self):
    ...
    var_fname = var_fname.strip() #this will remove any blank spaces before and after user's entry
    var_lname = var_lname.strip() #this ensures first character in each word is capitalized
    ...
    if not "@" or not "." in var_email: #forces email to have special format chars
...
#the > may need to be reversed below
def onDelete(self):
    ...
    with conn:
        ...
        #the database, cant delete the last record or we get an error
...
def onRefresh(self):
    ...
    self.lstList1.delete(0,END) #deletes everything in the listbox
    conn = sqlite3.connect('db_phonebook.db')
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT COUNT (*) FROM tbl_phonebook""") #count everything in the database
        count = cursor.fetchone()[0]
        i = 0
        while i < count: 
