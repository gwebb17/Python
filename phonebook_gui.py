...
def load_gui(self):
    #the self.master is where we are putting the label. 
    self.lbl_fname = tk.Label(self.master,text='First Name: ') #tk.Label is accessing prebuilt Tkinter class Label and we're naming it lbl_fname
    self.lbl_fname.grid(row=0,column=0,padx=(27,0),pady=(10,0),sticky=N+W) #now that we named it we can give it dimensions/properties and chose .grid
...
#----------------------------------------------------------------------------------------------------
    ...
    self.txt_phone.grid(row=5,column=0,rowspan=1,columnspan=2,padx=(30,40),pady=(0,0),sticky=N+E+W)#since it takes up more than one column we use columnspan
    ...
    self.lstList1.bind('<<ListboxSelect>>',lambda event: phonebook_func.onSelect(self,event)) #we are binding an EVENT/Listener to listen for clicks/selects
    self.scrollbar1.config(command=self.lstList1.yview)#(cont)..we will call the function onSelect
    self.scrollbar1.grid(row=1,column=5,rowspan=7,columnspan=1,padx=(0,0),pady=(0,0),sticky=N+E+S) #rowspan 7 is how far down the scrollbar can go
    self.lstList1.grid(row=1,column=2,rowspan=7,columnspan=3,padx=(0,0),pady=(0,0),sticky=N+E+S+W)

    #all the buttons we want to have
    #lambda is an anyonymous function, they are from our doc: phonebook_func. I think lambda is like {} placeholder for function
